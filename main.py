...
#     user = await client.http_client.get_user(message.from_user.id)
#     if user is None:
#         return await message.reply("Ты не зарегистрирован. Зайди в настройки чтобы зарегистрироваться.")
...
@bot.on_message(filters=filters.command('setting') | button_filter(buttons.setting_button))
async def setting_command(client: Client, message: Message):
    text = "Здесь настройки. Тут можно поменять свой никнейм."
    await message.reply (text, reply_markup=setting_keyboard)
...
async def sign_up_command(client: Client, message: Message):
    ...
    await message.reply(text, reply_markup = ForceReply(True) if user is None else setting_keyboard)
...
async def sign_up_reply(client: Client, message: Message):
    ...
    if validate is None:
        return await message.reply("Никнейм должен содержать от 3 до 9 символов и состоять из букв латинского алфавита и цифр.", reply_markup=setting_keyboard)

    user = await client.http_client.create_user(message.from_user.id, message.text)
    if user is None:
        return await message.reply("Не удалось зарегистрироваться. Попробуйте позже.", reply_markup=setting_keyboard)

    await message.reply("Регистрация прошла успешно!", reply_markup=setting_keyboard)



@bot.on_message()
async def unkown_message(client: Client, message: Message):
    await message.reply(
        'Неизвестная команда.'
        'Используйте кнопки внизу экрана, чтобы взаймодействовать с ботом.'
    )
